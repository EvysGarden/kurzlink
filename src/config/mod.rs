    ...
    pub fn render_files(
        ...
        for shortlink in &self.shortlinks {
            for source in &shortlink.sources {
                let source_render = render_redirect_html(&shortlink.destination, &template_path).unwrap_or_else(|_| panic!("Failed to run render_redirect_html for source {}", source));
